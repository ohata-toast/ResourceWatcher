...
リソースの変更事項はResource Watcherサービスで提供する通知ルールとして提供しています。
通知ルールはインスタンスの作成まだは起動、終了などのイベントが発生する場合にこれを検出し、
イベントが提供する詳細な情報を検出して通知ルールで設定した条件を満たすかどうかを判断できます。
通知を作成する時に通知ルールを選択して対象リソースを指定すると、該当リソースで通知ルールの条件に該当するイベントが発生した時にユーザーに通知を送ります。
...
| 項目 | 説明 |
|---|---|
| 通知検索条件 | 次のような検索条件を指定して該当するリソースを検索できます。<li>通知状態：有効化、無効化状態の通知を指定します。</li><li>通知ルール：通知ルールに該当する通知を検索します。通知ルールは複数の項目を選択できます。</li><li>通知名/通知説明検索：入力した検索ワードを含む通知を検索します。</li><li>通知最終修正日時：最後に修正された期間の通知を検索します。</li> |
...
通知の名前、説明および対象リソース、通知受信対象またはWebフック設定を変更できます。
- 通知ルールは変更できません。
